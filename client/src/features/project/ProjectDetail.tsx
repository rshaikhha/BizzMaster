...
export default function ProjectDetail() {
    ...
    useEffect(() => {
        agent.Projects.Get(parseInt(id)).then((res) => setSingle(res))
    }, [])
    ...
    return (
        <Grid container spacing={6}>
            <Grid item xs={12}>
                ...
                <Typography variant='body2'>Project: {single.title}</Typography>
                <Divider sx={{mb: 2}} />
                
                <Button variant="contained" sx={{m : 1 , minWidth : '200px'}} key="Back"component ={NavLink} to={`/Projects`}>Back</Button>
            </Grid>
            <Grid item xs={12}>
            <SimpleTable {... props} />
            </Grid>
